...
class System(object):
    ...
    def configSym(self,par) :           # config symbol data, par is a list 
        ...
        if s:
            ...
            print "configSym : not valid symbol", par[0]
    ...
    def show(self,q=0,ctx=None):
        if type(ctx) != list:           # symbol list for single context
            ...
            if not ctx:                 # default is current context
                ctx = self.curctx
            u = self.symList()
            s = ["Symbols in "+ctx.show()+" :"+\
                 format([e.ctx for e in u].count(ctx),'5d')]
            if q>=1:
    ...
    def inpLoop(self,init=title):   # interactive input loop for a session
        print init
        while True :                    # infinite loop
            ...
            if re.match(r'Quit',instr): # session ends with Quit[]
                break
            inexp = fun.ToExpr(instr,1) # input is parsed into expression
            ouexp = fun.Eval(inexp)     # evaluation
...
class Symbol(object):
    ...
    def show(self,t=None):
        ...
        if  (t/3) % 2 and self.flg.bit(oper):
            s += " "+format(self.opt,'7s')+"("+format(self.pre,'2d')+") "+\
            format(self.opr,'4s')
        if t % 3 == 2:
            s += " Flags: "+", ".join(self.flg.lis())    
        return s

# variables set flag integer (1 bit each one a power 2)
...
def init(fl):                   # initializes system structures from conf. file
    s = System()                    # system instantiation
    d = fun.ReadStr(fl)             # reads configuration data from file
    s.config(d)                     # configuration is done
    return s
